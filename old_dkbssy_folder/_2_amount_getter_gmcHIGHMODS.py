import datetime, sys, time, openpyxl
...
def emp_code(code):
    ...
    # Switch to the new tab and open the second URL
    driver.switch_to.window(driver.window_handles[1])

    driver.get(
        f"https://dkbssy.cg.nic.in/secure/incentivemodule/IncentiveDetails_EmpCodeWise_DetailsDME.aspx?ci={code}")
    # waiting for second page to load
    try:
        wait.until(EC.presence_of_element_located((By.XPATH, '/html/body/section/div/form/div[3]/div/div/div['
                                                             '2]/div/div/table/tbody/tr[2]/td[2]')))

        # getting table
        table = driver.find_element(By.XPATH, '//*[@id="ctl00_ContentPlaceHolder1_GridView1"]/tbody')
        table_text = table.text
        # print(table.text)

        # Connect to an in-memory SQLite database
        conn = sqlite3.connect(':memory:')
        # conn = sqlite3.connect(r'C:\Users\HP\Desktop\asd.db')
        cursor = conn.cursor()
        ...
        # Create a table
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS incentive_data (
                code TEXT,
                case_no TEXT,
                paid_amt REAL
            )
        ...
        for row in rows:
            ...
            cursor.execute('''
                INSERT INTO incentive_data (code, case_no, paid_amt)
                VALUES (?, ?, ?)
                ''', (code, cols[1], float(cols[2])))
        # Commit the transaction
        conn.commit()

        # Query to filter and sum the data
        query = ('''
            SELECT COUNT(*), SUM(paid_amt) 
            FROM incentive_data 
            WHERE case_no LIKE '%HOSP22G146659%'
            GROUP BY code
            ''')
        ...
        if result is not None:
            paid_done_count, paid_done_amount = result
        else:
            # Handle the case where no row is returned
            paid_done_count, paid_done_amount = 0, 0
        print('>>>> ', paid_done_count, paid_done_amount)
    ...
    # print(code, number_of_cases_pending, total_amount_pending.text, total_paid_cases.text, total_paid_amount.text, paid_done_amount)
    return code, number_of_cases_pending, total_amount_pending.text, total_paid_cases.text, total_paid_amount.text, paid_done_amount


login()


def excel_entry_update(number_of_emp_to_update=None):
    workbook_auto1 = openpyxl.load_workbook(r'G:\My Drive\GdrivePC\Hospital\RSBY\New\Incentive_auto2.xlsx')

    workbook_master = openpyxl.load_workbook(r'G:\My Drive\GdrivePC\Hospital\RSBY\New\Incentive_auto2.xlsx')
    worksheet_for_getting_names = workbook_auto1["Sheet3"]

    incentive_names_to_enter = []
    column_values = worksheet_for_getting_names.iter_rows(min_col=2, max_col=2, min_row=2, max_row=1026)
    ...
    if number_of_emp_to_update is not None:
        incentive_names_to_enter = incentive_names_to_enter[:number_of_emp_to_update]
    ...
    for name in incentive_names_to_enter:
        ...
        if name in dict_name_row_number.keys():
            ...
            e_code, pend_cases, pend_amount, total_cases, total_amount, paid_done_amount = emp_code(chosen_emp_code)
            ...
            worksheet_for_getting_id.cell(row=chosen_row_number, column=15).value = paid_done_amount
            worksheet_for_getting_id.cell(row=chosen_row_number, column=15).fill = fill_pattern_yellow
            ...
            print(name)
            print("THERE OCCURED AN ERROR, Check incentive auto 2")
            sys.exit()
