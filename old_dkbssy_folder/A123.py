...
def entry_proper_3(choosen_case_number, amount):  # WebdriverWait
    ...
    driver.find_element(By.XPATH,'//*[@id="ctl00_ContentPlaceHolder1_GridView1_filter"]/label/input').send_keys(choosen_case_number)
    try:
        choosen_incentive_case = wait.until(EC.presence_of_element_located((By.XPATH, '//*[@id="ctl00_ContentPlaceHolder1_GridView1"]/tbody/tr/td[2]/a'))).text
        ...
        if choosen_incentive_case == choosen_case_number:
            driver.get(f"https://dkbssy.cg.nic.in/secure/incentivemodule/incentivedetailsdme.aspx?c={choosen_case_number}&amt={amount}")
    except:
    ...
    for categ in category_all:
        ...
        if len_names == 0:  # skipping the empty category e.g. Anaesthesia
            ...
            for num in range(number + 1 - len_names, number + 1):
                ...
                name_amount = table_in_name.text + '@' + str(percentage * float(incentive_amount.text) / len_names)
                print(name_amount)
    ...
    try:
        for inc_nam in final_incentive_names[6:]:
            cur.execute('''
                INSERT INTO incentive_entry_T (employee_name, incentive_case_number, incentive_amount, timestamp)
                VALUES (?,?,?,?)''',(inc_nam.split('@')[0],final_incentive_names[0],inc_nam.split('@')[1], current_timestamp))
...
def names_for_incentive() -> list:
    ...
    incentive_names_to_enter = []
    column_values = worksheet_for_getting_names.iter_rows(min_col=3, max_col=12, min_row=2, max_row=1026)
...
def updating_amount():
    ...
    results = sqlite_process()  # function ran
    workbook_master = openpyxl.load_workbook(r'G:\My Drive\GdrivePC\Hospital\RSBY\New\Incentive_auto2.xlsx')
    worksheet_for_getting_id = workbook_master["Sheet3"]
    driver.get('https://dkbssy.cg.nic.in/secure/incentivemodule/IncentiveDetails_EmpCodeWiseDME.aspx')
    for name in incentive_names_to_enter:
