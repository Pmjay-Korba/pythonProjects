...
class AllListGenerator:
    ...
    def retrieve_date_new_claim_query(self, page: Page, case_number):
        ...
        if current_status == 'Preauthorization Queried' or current_status == 'Pre-Authorization Queried':
            ...
            # ColourPrint.print_yellow('bbb')
            # time.sleep(3)
            # main treatment already opened clicking treatment plan
            '''This below step has caused issue in recent. it closes the treatment which hinder getting the diagnosis in next step'''
                ...
                if not is_query_question_required:
                    ...
                    try:
                        try:# clicking SHOW MORE
                            ...
                            recent_query_date_time = page.locator(recent_query_date_xp).text_content()  # 22/03/2025 12:17:21
                            ...
                            try:
                                ...
                                page.wait_for_selector("(//img[@data-tip='Chat'])[last()]", timeout=2000).click()

                                # getting question - last question
                                question = page.wait_for_selector(
                                    "(//div[@class=' mt-1 GoMHIRgbsMpPdNHZPHNf']/span)[last()]").text_content().strip()
                                # print('---------->',question)


                                recent_query_date_xp_fin = "(//div[@class=' mt-1 GoMHIRgbsMpPdNHZPHNf']/div/p)[last()]"
                                recent_query_date_time = page.locator(recent_query_date_xp_fin).text_content()  # 03/22/2025, 12:16
                                recent_query_date_only = recent_query_date_time.split(',')[0]
                                # modify date
                        ...
                        ColourPrint.print_pink('Here in Exception')
                        # click Finance
                        page.wait_for_selector("//h4[normalize-space()='Finance']/parent::button").click()
                        # select the query chat option and click it to open chat window
                    ...
                    case_num_data.append('Ward_Name_is')
                    case_num_data.append(recent_query_date)
