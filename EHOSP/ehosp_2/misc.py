...
def automate_discharge_with_manual_token(ward_id, token, single_dischargeable_ipd:list, whole_active_patients_data ):
    ...
    ipd_number:int = validate_ipd_as_correct_in_excel(single_dischargeable_ipd[0])
    ...
    '''
    # 📋 Step 2: Get discharge types
    discharge_types_resp = request.get(
        "https://nextgen.ehospital.gov.in/api/ipd/doc/dischargetype",
        headers=headers
    )
    # With this:
    if discharge_types_resp.ok:
        discharge_types = discharge_types_resp.json()
        # print(discharge_types)
    ...
    # getting the names of ward and its ids
    # ward_names_resp = request.get("https://nextgen.ehospital.gov.in/api/ipd/common/wardMaster/7013",
    #                               headers=headers)
    # if ward_names_resp.ok:
    #     ward_json = ward_names_resp.json()
    #     ColourPrint.print_yellow('='*25)
    #     print(ward_json['metadata'])
    #     # for i in ward_json['result']:
    ...
    patient_list_resp = request.get(f'https://nextgen.ehospital.gov.in/api/ipd/common/patientByWard/7013/{ward_id}',
                                    headers=headers)
    ...
    if patient_list_resp.ok:
        ...
        for each_patient_details in patient_details:  # each patient details from all patient list in ward pending discharges
            if str(ipd_number) in each_patient_details["admno"]: # USE STRIP HERE
                ...
                'getting all discharges pending of a ward --- This if from PREPARE SUMMARY PAGE after INITIATION PAGE discharge type update'
                ...
                check_discharge_date_range(each_patient_summary_json, single_dischargeable_ipd)



    # summary_entry(each_patient_summary_json=each_patient_summary_json,
    #               user_id=USER_ID,
    #               discharge_date_is=DISCHARGE_DATE_IS,
    #               request=request,
    #               token=token)
...
def check_discharge_date_range(each_patient_summary_json:dict, single_dischargeable_ipd:list): #  one of dischargeable_ipds = [
    ...
    print('adm_dt', datetime_admission_date, 'dis_dt', datetime_discharge_date)
    if datetime_admission_date<datetime_discharge_date<datetime.datetime.now()+datetime.timedelta(minutes=2):
        print('ok')


def summary_entry(each_patient_summary_json:dict, user_id, discharge_date_is, request, token):
    if not each_patient_summary_json["brief_summary"] :
        payload = {
            "summary": {
                ...
                "pat_condition_on_dis": f"{processed_disharge_condition}",
...
def select_matching_ipd_from_active_patient_list(ipd_number, whole_active_patient_list ):
    for individual_patient_data in whole_active_patient_list:
        ...
        if individual_patient_data["ipdid"] == str(ipd_number).strip():
            print(individual_patient_data)
...
def fetch_excel(workbook_path):
    ...
    print(user_id_of_excel,ward_id_of_excel,ward_of_excel)
    ...
    diagnosis_dict ={i[0].lower():i[1].strip() for i in diagnosis_rows}
    ColourPrint.print_yellow('--'*50)
    ...
    treatment_rows = list(treatment_sheet.iter_rows(min_row=2, values_only=True))
    ...
    return ward_of_excel, ward_id_of_excel, user_id_of_excel, dischargeable_ipd
    ...
    if discharge_date:
        ...
        if isinstance(discharge_date, datetime.datetime):
            ...
            if discharge_date.lower() == 't':
                discharge_dt = datetime.datetime.now()
                print(discharge_dt)
...
if __name__ == "__main__":
    WARD_ID = 4057
    # getting the user ward and discharges = user_id_of_excel='Neha_701336', ward_id_of_excel=" 'Female Ward'", [[2017237, '15.11.17', 11, 'aa', None], [2017287, '15.110.2.17', 11, 'aa', None]]
    # WARD_NAME, WARD_ID, USER_ID, discharge_list = fetch_excel(r"G:\Other computers\My Computer\pythonProject\EHOSP\ward_discharge_entry.xlsx")

    # this is the correct below line

    # print(fetch_excel(r"G:\Other computers\My Computer\pythonProject\EHOSP\ward_discharge_entry.xlsx"))

    #
    # print(USER_ID, WARD_ID, discharge_list)

    IPD_NO = "20171695"
    # DISCHARGE_DATE_IS =  # required style = "2017-11-21 16:33:00"
    ...
    with sync_playwright() as p:
        ...
        ward_of_excel, ward_id_of_excel, user_id_of_excel, dischargeable_ipd = fetch_excel(
            r"/EHOSP/ward_discharge_entry.xlsx")
        ...
        for single_dischargeable_ipd in formatted_final_dischargeable_ipds_except_datetime:
            print('Single Discharge IPD: ', single_dischargeable_ipd)
            # selecting the ipd number patient fron whole active data

            automate_discharge_with_manual_token(ward_id= WARD_ID, token=token, single_dischargeable_ipd=single_dischargeable_ipd, whole_active_patients_data=whole_active_patients_data)

        # print(process_date_time())
