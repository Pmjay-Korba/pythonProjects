...
USER = {
 "USER663760":"Surindar Lal Chandra",
 "USER7490931":"Veena",
 "USER676549":"Sangeeta",
 "USER7700876":"Jyoti Lahre",
 "USER6480128":"Sumitra Kurrey",
 "USER7490958":"Yamini Verma",
 "USER7728697":"Ramakant",
 "USER7741848":"Mithilesh",
 "USER7755274":"Yogendra Khute",
 "USER7743625":"Vivek Kanwar",
 "USER6327065":"KIRAN SAHU",
 "USER268299":"PRAKASH SIDAR",
 "USER268312":"Usha Shrivas",
 "USER6523003":"KAMLESH MARKAM",
 "USER6948197":"USER6948197",
 "USER6523":"USER6523",
 "USER645375":"USER645375"
...
class AsyncTms:
    ...
    PREAUTHORIZATION_QUERY_SEARCH_VALUE ='12'
    CLAIM_QUERY_SEARCH_VALUE = '21'
    BLOCKING_REMAINING_SEARCH_VALUE = '15'
    DOWNLOAD_DIR = str(Path(__file__).resolve().parent.parent / 'async_tms_new' / 'download')
    EXCEL_FILE_PATH = DOWNLOAD_DIR + "\\" + "AYUSHMAN REGISTRATION 2025.xlsx"


    async def main_main(self):
        async with async_playwright() as p:
            browser = await p.chromium.connect_over_cdp(f"http://localhost:{self.CDP_PORT}")
            context = browser.contexts[0]
            ...
            # generating the lists
            blocking_remining = await self.get_lists_of_pendings(
                url=self.BENEFICIARY_URL,
                search_value=self.BLOCKING_REMAINING_SEARCH_VALUE,
                context=context,
                headers=headers
                )
            ...
            under_treat = await self.get_lists_of_pendings(
                url=self.BENEFICIARY_URL,
                search_value=self.UNDER_TREATMENT_SEARCH_VALUE,
                context=context,
                headers=headers
            ...
            both_query_list = blocking_remining
            print('BOTH QUERY', both_query_list)
            ...
            #                                                                      sheet_name_to_be_compared='QUERY2')
            #
            all_data = await self.process_all(both_query_list=both_query_list,context=context,headers=headers)
            print(all_data)
            sum_data = {}
            for data in all_data:
                user_is = USER.get(data[2],'Unknown')
                print(data[0], data[1][:10], USER.get(data[2],'Unknown'))
                if not user_is in sum_data:
                    sum_data[user_is] = 1
                else:
                    sum_data[user_is] += 1
            print('pending_blocking')
            print(sum_data)


            all_data = await self.process_all(both_query_list=claim_query_list_generated, context=context, headers=headers)
            print(all_data)
            sum_data = {}
            for data in all_data:
                user_is = USER.get(data[2],'Unknown')
                ...
                if not user_is in sum_data:
                    ...
                    sum_data[user_is] += 1
            print('claim query list')
            print(sum_data)

            all_data = await self.process_all(both_query_list=pre_auth_query_list_generated, context=context,
                                              headers=headers)
            print(all_data)
            sum_data = {}
            for data in all_data:
                ...
                if not user_is in sum_data:
                    ...
                    sum_data[user_is] += 1
            print('preauth_ query List')
            print(sum_data)

            all_data = await self.process_all(both_query_list=under_treat, context=context, headers=headers)
            print(all_data)
            sum_data = {}
            for data in all_data:
                user_is = USER.get(data[2],'Unknown')
    ...
    async def get_user_ids(self, registration_no, context, headers):
        # user_id=
        # date=
        URL =f'https://apisprod.nha.gov.in/pmjay/provider/provider/activity/log?registrationid={registration_no}'
        r = await context.request.get(url=URL, headers=headers)
        res = await r.json()
        # print(res)
        first = res[0]
        print(registration_no, first ['raiseddate'], first['user'])
        return registration_no, first ['raiseddate'], first['user']
    ...
    async def each_case_number_details(self, registration_no, context, headers):
        ...
        card_is, name_is, department_is, procedure_is, sex_is, amount_is = await self.retrieve_treatment_details(registration_no=registration_no, context=context,headers=headers)
        registration_date_is, last_status, last_remark, last_remark_date = await self.retrieve_logs(registration_no=registration_no, context=context,headers=headers)
        age_is, newborn = await self.retrieve_beneficiary_info(registration_no=registration_no, context=context,headers=headers)

        age_sex_newborn_is = str(age_is) +" Yr "+ sex_is +" "+ newborn

        print(registration_no, card_is, name_is, registration_no, registration_date_is, department_is, procedure_is, age_sex_newborn_is, amount_is, last_status,last_remark_date,last_remark)
        return card_is, name_is, registration_no, registration_date_is, department_is, procedure_is, age_sex_newborn_is, last_status, amount_is, '__PENDING DAYS__', last_remark,'__WARD NAME__',last_remark_date
    ...
    async def retrieve_treatment_details(self, registration_no, context, headers) -> tuple[str,str,str,str,str,str]:
        ...
        sex_is = r_data['encounter']['patientgender']
        amount_is = r_data['amount']['claimedamount']
        # ColourPrint.print_blue(card_is,name_is,registration_data_is,department_is,procedure_is,sex_is,amount_is)
        return card_is,name_is,department_is,procedure_is,sex_is,amount_is
...
if __name__ == '__main__':
    asyncio.run(AsyncTms().main_main())
